...
for n = 1:num_iter
    ...
    T = GaussScidelSolver(T, nodesx, nodesy, dx, dy, Nx, Ny, Lx, Ly, S, q);
    disp(T)
end

function T = GaussScidelSolver(T, nodesx, nodesy, dx, dy, Nx, Ny, Lx, Ly, S, q)
    % Boundary Conditions for 1 and 3
    T(1, :) = 15;
    T(end, :) = 10;
    for i = 2:Ny+1
        dist_btw_nodesy_up = nodesy(i+1) - nodesy(i);
        dist_btw_nodesy_down = nodesy(i) - nodesy(i-1);
        y = nodesy(i) - nodesy(1);
        ...
        for j = 2:Nx+1
            ...
            dist_btw_nodesx_front = nodesx(j+1) - nodesx(j);
            dist_btw_nodesx_back = nodesx(j) - nodesx(j-1);
            delx = dx(j);
            ...
            if j == 2
                T(i, 1) = T(i, 2) - (q/k) * dist_btw_nodesx_back;
            end
